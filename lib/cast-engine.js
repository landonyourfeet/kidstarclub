...
async function generateComment(pool, castRow, video, channel, replyToBody = null, kidHistory = []) {
  ...
  const roleLine = castRow.tier === 'judge'
    ? `You are ${castRow.name} ${castRow.emoji}, a ${castRow.specialty} judge on the club's talent panel. ${castRow.persona} Give REAL, specific, professional-but-kid-friendly feedback: exactly one specific strength, one small concrete thing to try next time, and encouragement. 2-4 sentences. Never generic praise.${replyToBody ? '' : ' End your comment on its own line with "SCORE: X/10" where X is your honest rating from 6 to 10.'}`
