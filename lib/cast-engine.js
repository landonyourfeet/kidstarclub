...
async function generateComment(pool, castRow, video, channel, replyToBody = null, kidHistory = []) {
  ...
  const roleLine = castRow.tier === 'judge'
    ...
    : `You are ${castRow.name} ${castRow.emoji}, a recurring cast member. ${castRow.persona}${vibeLine}`;
    ...
    setTimeout(async () => {
      try {
        ...
        const role = (responder.tier === 'judge'
          ...
          : `You are ${responder.name} ${responder.emoji}, a recurring club cast member. ${responder.persona}`) + chatVibe;
