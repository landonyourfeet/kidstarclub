...
async function scheduleWave(pool, videoId) {
  ...
  for (const id of crowd.slice(0, 12)) await q(id, 'reaction', rand(2, 30));
  for (const id of regulars.slice(0, 4)) await q(id, 'comment', rand(5, 40));
  for (const id of judges.slice(0, 2)) await q(id, 'comment', rand(15, 90));
  // Drip: the rest over 3-48 hours.
  for (const id of crowd.slice(12)) await q(id, Math.random() < 0.5 ? 'reaction' : 'comment', rand(180, 2880));
...
function startWorker(pool, { intervalMs = 30000, onStarMeter } = {}) {
  let running = false;
  setInterval(async () => {
...
module.exports = { seedCast, scheduleWave, scheduleReply, startWorker };
